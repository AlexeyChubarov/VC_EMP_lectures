...
\end{equation*}
Как и при изучении оператора $L$ в $\mc{D}_L$\footnote{\begin{equation*}
...
\end{equation}
Это и есть уравнение для нахождения $z$, а значит {\mbи} $\omega${\mb,} и $\lambda$.
...
\end{equation*} \footnotetext{$k=0$ --- рассматривали раньше.}откуда
\begin{equation*}
...
{\mbВернёмся} к решаемому уравнению~\eqref{l7:eq:11} и обозначим его правую часть через $\Psi(z)$:
\begin{equation}
	\label{l7:eq:11a}
...
Рассмотрим{\mb,} наконец{\mb,} общий случай $\gamma_1\cdot\gamma_2>0$. В этом случае функция $\Psi(z)$ имеет разрыв при $z=z_0=l\cdot\sqrt{\gamma_1\cdot\gamma_2}$, и $\Psi(z_0+0)\?=+\infty$, $\Psi(z_0-0)\?=-\infty$.
\begin{figure}[H]\centering
...
\end{multline*} 
В силу граничных условий {\mbвне} интегральные члены равны нулю, так что никаких дополнительных граничных условий не возникает, поэтому при допустимых $\eta(x)$
\begin{equation*}
