...
Если $y$ --- минимайзер и $\eta$ --- допустимое изменение, то $\eta(a)=\eta(b)\?=0$, и в силу \eqref{l1:eq:7} для минимайзера $y$ и $\forall\eta$ {\mb 
%допустимого
}
\begin{equation}
	\label{l1:eq:10} 
...
\begin{proof}[Доказательство леммы Лагранжа]
	...
	Предположим, что \eqref{l1:eq:12} верно при $\forall\eta$ и при некоторой функции $\Phi(x)\not\equiv0$. Тогда $\exists x_0${\mb,} такое, что $\Phi(x_0)\neq0$. Не ограничивая общности{\mb,} считаем, что $a<x_0<b$ и что $\Phi(x_0)>0$. Так как $\Phi(x)\in\Cfn[{[a,b]}]{}$, то $\exists\delta>0${\mb,} так что $\Phi(x)>0$ при $x\in {[x_0-\delta,x_0+\delta]}$. Положим
	\begin{equation*}
...
Тогда
	...
	\end{equation}
	ибо на интервале $(x_0-\delta,x_0+\delta)$ функции $\Phi(x)$ и $\eta(x)$ положительн{\mbы}. В то же время, так как $\eta(x)$ --- допустимое изменение, то{\mb,} по условию леммы{\mb,} выполняется \eqref{l1:eq:12}. Значит, \eqref{l1:eq:13} неверно{\mb,} и причина этого --- предположение о том, что $\exists x_0${\mb,} такое, что $\Phi(x_0)\neq0$. Таким образом{\mb,} $\Phi(x)\equiv0$.
\end{proof}
...
{\noindent\centering\begin{tabular}{p{0.47\textwidth}|p{0.47\textwidth}}
	...
	Задано $y(a)=y_0$, $y'(a)=y_0'$. Другими словами{\mb,} задано значение $y(a)$ и наклон касательной $y'(a)$
