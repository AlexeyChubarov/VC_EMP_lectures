...
Примеры:
\begin{enumerate1}
	\item $S[f]=\int\limits_a^b f(x)\,dx$ --- площадь заштрихованной фигуры, $f(x)\in \Cfn[{[a,b]}]{}$.
	...
	\item $l[f]=\int\limits_a^b\sqrt{1+f^{\prime 2}(x)}\,dx$ --- длина кривой, $f\in \Cfn[{[a,b]}]{1}$ --- класс непрерывно дифференцируемых на ${[a,b]}$ функций.
\end{enumerate1}
...
\begin{equation*}
	\hfill \delta\J=t\left\{\int\limits_a^b\left(F_y-\der{}{x}F_{y'}\right)\cdot\eta\,dx+F_{y'}\cdot\eta\mathop{\Big|}\limits_a^b\right\}\hfill\tag{A}
\end{equation*}
...
\begin{equation}
	\label{l1:eq:10}
	\hfill t\int\limits_a^b\left(F_y-\der{}{x}F_{y'}\right)\cdot\eta\,dx=0\hfill
\end{equation}
...
\begin{equation*}
\text{Пусть }\J[y]=\int\limits_0^1 x^2\cdot y^{\prime 2}\,dx\quad \mc{K}=\left\{y(x)|y\in\Cfn[{[0,1]}]{1}, y(0)=1, y(1)=0\right\}
\end{equation*}
Ясно, что $\J[y]>0, y\in\mc{K}$. Покажем, что $\inf\limits_{y\in\mc{K}}\,\J[y]=0$.
\begin{equation*}
	\text{Рассмотрим функции }y_{\alpha}(x)=\begin{cases}
		(x-\alpha)^2\cdot\frac1{\alpha^2}&0\leqslant x\leqslant\alpha\\
		0&\alpha\leqslant x\leqslant1
...
\end{tikzpicture}
\begin{equation*}
	\J[y_{\alpha}]=4\int\limits_0^{\alpha} x^2\cdot\frac{(x-\alpha)^2}{\alpha^4}\,dx\leqslant\frac4{\alpha^2}\int\limits_0^{\alpha} (x-\alpha)^2\,dx=\frac4{3\cdot\alpha^2}(x-\alpha)^3\mathop{\Big|}\limits_0^{\alpha}\to0\text{ при }\alpha\to0
\end{equation*}  
