...
\begin{proof}[Доказательство теоремы~\ref{l5:s2:teor:3}.]
	...
	\begin{gather*}
		...
		2\cdot P\cdot y_p-2\cdot\lambda_p\cdot y_p=\sum\limits_{j=1}^{p-1}\mu_j\cdot y_j-2\cdot\der{}{x}\big(Q\cdot y'_p\big)=0,
	\end{gather*}  
	откуда
	\begin{equation}
		\label{l5:eq:10}
		\hfill Ly_p=-\der{}{x}\big(Q\cdot y'_p\big)+P\cdot y_p=\lambda_p\cdot y_p+\sum\limits_{j=1}^{p-1}\frac{\mu_j\cdot y_j}{2}.\hfill
	\end{equation}
